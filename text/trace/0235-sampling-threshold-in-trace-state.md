...
2. A 56-bit _drop threshold_ (or just "threshold") as expressed in the TraceState, called `T` below. `T` represents the minimum threshold that was applied in all previous consistent sampling stages. If the current sampling stage applies a lower threshold than any stage before, it MUST update (decrease) the threshold correspondingly.
