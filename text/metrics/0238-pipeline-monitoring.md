...
OpenTelemetry SDKs and Collectors with support for multiple levels of
detail.
...
that was intentionally not transmitted or that has failed and cannot
be retried.  A survey of existing pipeline components shows the
following uses.
...
## Meaning of "rejected" telemetry

TODO: https://github.com/open-telemetry/opentelemetry-collector/pull/9260#discussion_r1473516302
...
- `consumed`: Indicates a normal, synchronous request success case.
  ...
  returned success.
- `enqueued`: Indicates the component admitted items into a queue and
  then allowed the request to return success before the final outcome 
  was known.
- `suppressed:<success=false outcome>`: When the outcome is known but
  the compnent intentionally returns.
...
For both success=true and success=false: 

- `dropped`: Indicates an item that was not sent to the next stage of
  the pipeline in both cases.  Processors may use this to indicate
  both success and failure, for example include sampling processors
  and filtering processors, which successfully avoid sending data
  based on configuration.  For receivers and exporters, dropped data
  indicates resource exhaustion of the component itself, in which case
  the component reports points as dropped, while the producers will
  see a resource exhausted status code.
